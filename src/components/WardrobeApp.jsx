...
import { ensureProfile } from '../utils/friends';
...
function SortableItemCard({ item, onEdit, onDelete }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="group cursor-pointer" onClick={() => onEdit(item)}>
      <div className="aspect-[3/4] w-full overflow-hidden relative mb-3" style={{ backgroundColor: '#f5f0eb' }}>
        {item.image
          ...
          : <div className="w-full h-full flex items-center justify-center"><span className="text-4xl opacity-15">{cat?.emoji}</span></div>
        }
        ...
        <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          >
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2"><path d="M8.5 1.5l2 2L4 10H2v-2L8.5 1.5z"/></svg>
          </button>
          <button
            ...
            style={{ backgroundColor: '#FAF8F5', color: '#a09890', boxShadow: '0 1px 4px rgba(0,0,0,0.1)' }}
          >
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.2"><path d="M2 3h8M5 3V2h2v1M3 3l.5 7h5L9 3"/></svg>
          </button>
        </div>
      </div>
      <div>
  ...
  return (
    <div className="mb-14">
      <div className="flex items-center gap-4 mb-6">
        <h2 style={{ ...serif, fontSize: '18px', fontWeight: 400, color: '#1a1713', fontStyle: 'italic', letterSpacing: '-0.01em' }}>{cat?.label}</h2>
...
// ─── Feed item card (read-only, shows friend name) ───────────────────────────
...
function FeedItemCard({ item }) {
  ...
  return (
    <div className="group">
      ...
      <div>
        <p style={{ ...sans, fontSize: '10px', color: '#c9b99a', letterSpacing: '0.08em', marginBottom: '3px', textTransform: 'uppercase' }}>
...
export default function WardrobeApp({ user, displayName, onLogout }) {
  ...
  const [view, setView] = useState('closet'); // 'closet' | 'friends'
  ...
  useEffect(() => {
    ensureProfile(user, displayName).catch(console.error);
    loadItems();
  }, []);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#FAF8F5', fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="sticky top-0 z-20" style={{ backgroundColor: '#FAF8F5', borderBottom: '1px solid #e8e3de' }}>
        ...
        {/* Nav tabs row */}
        <div className="max-w-7xl mx-auto px-6 flex gap-0" style={{ borderTop: '1px solid #f0ebe3' }}>
          {navTabs.map(t => {
            ...
            return (
              <button
                ...
                className="relative shrink-0 px-5 py-3 transition-all"
                style={{
                  ...sans,
                  fontSize: '11px',
                  letterSpacing: '0.1em',
              ...
              >
                ...
                {t.id === 'friends' && pendingCount > 0 && (
                  <span
                    className="absolute top-2 -right-0.5 w-4 h-4 flex items-center justify-center rounded-full text-white"
                    style={{ ...sans, fontSize: '9px', backgroundColor: '#c9b99a' }}
                  >
                    {pendingCount}
                  </span>
                )}
      ...
      {/* Category & occasion filters — closet only */}
      {view === 'closet' && (
        <>
          <div style={{ backgroundColor: '#FAF8F5', borderBottom: '1px solid #e8e3de' }}>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-10">
        ...
        {/* ── Friends ── */}
        {view === 'friends' && (
          <FriendsTab onPendingCountChange={setPendingCount} />
        )}
